...
// - OpenStreetMap の駅周辺 POI (lib/osm-pois.js): Wikipedia に記事の無い川・橋・寺社・店など
// MediaWiki API (ja.wikipedia) を使う。同名駅は座標で照合し、記事が無い駅は素材なしとして扱う。

import { collectPois } from "./osm-pois.js";
...
/**
 * 記事名が実在するかを調べる。リダイレクトは転送先で判定する。
 * 問い合わせに失敗した記事名は結果に入れない (「無い」と「分からない」を区別するため)。
 * @param {string[]} titles
 * @returns {Promise<Map<string,boolean>>}
 */
export async function checkTitles(titles) {
  const result = new Map();
  for (let i = 0; i < titles.length; i += 50) {
    ...
    for (const orig of batch) {
      ...
      result.set(orig, exists.has(canonical));
    }
 ...
 * @returns {Promise<{lineInfo:string|null, stationSpots:{name:string,info:string,around:string,history:string,links:string[],nearby:{title:string,km:number}[],pois:{name:string,kind:string,m:number,note?:string}[]}[]}>}
 */
export async function collectSpots(line, stations) {
  // OSM は Overpass の待ちが長いので、Wikipedia を引いている間に並行して集める
  const poisPromise = collectPois(stations);
  ...
  for (const s of stations) {
    ...
    if (!pg) {
      stationSpots.push({ name: s.name, info: "", around: "", history: "", links: [], nearby, pois: [] });
      continue;
    }
  ...
  stationSpots.forEach((sp, i) => (sp.pois = pois[i] || []));
