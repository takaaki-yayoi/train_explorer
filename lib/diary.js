 ...
 */
function buildPrompt({ line, persona, stations, spots, date, weather }) {
...
# 生成ルール
- 日記は時系列。各エントリは type が "stop"(途中下車, 長文) か "pass"(車窓から, 短文)。
- stop は3〜6文でエピソード性を持たせる。pass は1〜2文で軽く。
- 起点駅と終点駅は必ず stop。それ以外は魅力的な駅を2〜3つ stop にし、残りは pass。
- ローカル線ではダイヤの疎らさを反映する(途中下車したら次の列車まで数時間、という時間経過を入れるとリアルになる)。
- ${persona.name}の性格を通して「何に目を留めるか」を変える。${persona.interests}に沿った着眼で。
- 全エントリで st は 0 から ${stations.length - 1} まで昇順(重複可)。時刻 t も昇順。
- 全体で駅数と同程度〜少なめのエントリ数。
...
export async function generateDiary(ctx, opts = {}) {
  ...
  const out = provider === "anthropic"
    ? await callAnthropic(system, user, opts)
    : await callOpenAI(system, user, opts);
  const diary = sanitizeDiary(out.diaryRaw, ctx.stations.length);
  return { diary, model: out.model, usage: out.usage, provider };
}
