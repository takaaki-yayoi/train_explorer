...
let _font = null;
let _icon = null;
// トップOGカードに載せるロゴアイコン (data URI)
function iconDataUri() {
  if (_icon === null) {
    try {
      _icon = "data:image/png;base64," + readFileSync(join(ROOT, "public/icon-512.png")).toString("base64");
    } catch {
      _icon = "";
    }
  }
 ...
 */
export function tripSvg(trip, siteHost = "bunshin-tabi.com") {
...
${icon ? `<image href="${icon}" x="536" y="72" width="128" height="128"/>` : ""}
<text x="600" y="322" text-anchor="middle" font-family="Noto Sans JP" font-weight="700" font-size="72" fill="#ffffff">分身の旅日記</text>
<text x="600" y="384" text-anchor="middle" font-family="Noto Sans JP" font-weight="400" font-size="32" fill="#c9c2b6">分身が日本の鉄道を旅した記録。毎朝1本、新しい路線へ。</text>
<text x="600" y="480" text-anchor="middle" font-family="Noto Sans JP" font-weight="700" font-size="40" fill="#e8657c">${stats.trips || 0}便 ・ ${stats.lines || 0}路線 ・ 累計${Math.round(stats.km || 0)}km</text>
</svg>`;
}
