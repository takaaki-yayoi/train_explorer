...
async function ensure() {
  if (!_inited) {
    ...
    _font = readFileSync(join(ROOT, "assets/NotoSansJP.ttf"));
  }
  await _inited;
}
...
<text x="60" y="298" font-family="Noto Sans JP" font-weight="400" font-size="30" fill="#6b6459">${esc(sub)}</text>
<text x="60" y="360" font-family="Noto Sans JP" font-weight="700" font-size="30" fill="#e8657c">${esc(who)}</text>
<text x="60" y="410" font-family="Noto Sans JP" font-weight="400" font-size="26" fill="#9a948b">${esc(trip.date || "")}${trip.weather ? " ・ " + esc(trip.weather) : ""}</text>
<text x="60" y="596" font-family="Noto Sans JP" font-weight="400" font-size="22" fill="#b0a89c">train-explorer.noteigi.workers.dev</text>
</svg>`;
}
